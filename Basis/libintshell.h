...
#ifndef LIBINTSHELL_H
#define LIBINTSHELL_H
...
namespace {
/// df[k] = (k-1)!!
static constexpr std::array<int64_t, 31> df
    = {{1,               1,                1,               2,
        3,               8,                15,              48,
        105,             384,              945,             3840,
        10395,           46080,            135135,          645120,
        2027025,         10321920,         34459425,        185794560,
        654729075,       3715891200,       13749310575,     81749606400,
        316234143225,    1961990553600,    7905853580625,   51011754393600,
        213458046676875, 1428329123020800, 6190283353629375}};
}

/// generally-contracted Solid-Harmonic/Cartesion Gaussian Shell

/** Simple-to-use Gaussian shell. Here's an example of how to create an s+p
 * shell of the STO-3G basis on the oxygen at
 ...
 * primitives. Basis Set Database,
 *  as well as basis set libraries embedded into most quantum chemistry programs
 * use this convention.
 *  However, coefficients must be converted to refer to normalization-free
 * primitives before computing
...
struct Shell {
  ...
  /// before computing integrals.
  /// \note Must be done only once.
  void renorm() {
    const auto sqrt_Pi_cubed
        = LIBINT2_REALTYPE{5.56832799683170784528481798212};
    const auto np = nprim();
    for (auto &c : contr) {
      ...
      for (auto p = 0; p != np; ++p) {
        ...
        const auto two_alpha_to_am32 = pow(two_alpha, c.l + 1)
                                       * sqrt(two_alpha);
        const auto norm = sqrt(pow(2, c.l) * two_alpha_to_am32
                               / (sqrt_Pi_cubed * df[2 * c.l]));
