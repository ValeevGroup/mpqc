...
int main(int argc, char** argv){
  ...
  // Need to reduce the rank of our added vectors find there real rank, not 2 * original rank.
  qr.compute(La);
  ...
  MatrixXd LaR = qr.matrixR().topLeftCorner(qr.rank(), qr.matrixQR().cols()).template triangularView<Upper>();
  LaR *= qr.colsPermutation().transpose();
  MatrixXd RaR = qrR.matrixR().topLeftCorner(qrR.rank(), qrR.matrixQR().cols()).template triangularView<Upper>();
  RaR *= qrR.colsPermutation().transpose();

  // New low rank L and low rank R.  Arbitrarrialy picked L to absorb the small matrix.
