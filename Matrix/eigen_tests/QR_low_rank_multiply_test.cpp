...
int main(int argc, char **argv) {
  ...
  MatrixXd R = qr.matrixR()
                   .topLeftCorner(qr.rank() + 1, qr.matrixQR().cols())
  ...
  double reduced_time = std::chrono::duration_cast
  ...
  double full_time = std::chrono::duration_cast
                     <std::chrono::duration<double>>(m1 - m0).count();

  std::cout << input << ", " << qr.rank() << ", " << qr_time << ", "
            << reduced_time << ", " << qr_time + reduced_time << ", "
            << full_time << "\n";
