...
class LazyIntegral {
  ...
  public:
    ...
    /// Constructor
    LazyIntegral(range_type range, const std::vector<std::size_t> &index,
                 std::shared_ptr<IntegralGenerator> integral_generator)
            : range_(range),
              index_(index),
              integral_generator_(integral_generator) {
...
typedef tcc::cc::LazyIntegral<4, TwoBodyIntGenerator<libint2::Coulomb>>
      LazyTwoElectronTile;
typedef TA::Array<double, 4, LazyTwoElectronTile, TA::DensePolicy>
      DirectTwoElectronDenseArray;

// function to make direct two electron dense TArray
DirectTwoElectronDenseArray
make_lazy_two_electron_array(madness::World &world,
                             const tcc::basis::Basis &basis,
                             const TA::TiledRange &trange) {

    auto p_cluster_shells
          = std::make_shared<std::vector<tcc::basis::ClusterShells>>(
                basis.cluster_shells());

    auto two_body_coulomb_engine = tcc::integrals::make_2body(basis);

    auto p_engine_pool = std::
          make_shared<tcc::integrals::
                            EnginePool<libint2::
                                             TwoBodyEngine<libint2::Coulomb>>>(
    ...
    for (; it != end; ++it) {
        TA::Range range
              = lazy_two_electron.trange().make_tile_range(it.ordinal());
        auto index = it.index();
        lazy_two_electron.set(index,
                              LazyTwoElectronTile(range, index,
...
typedef tcc::cc::LazyIntegral<4, TwoElectronIntDFGenerator<TA::DensePolicy>>
