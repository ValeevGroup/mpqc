...
AC_DEFUN([ACX_CHECK_SHARED_PTR], [
  ...
  AC_COMPILE_IFELSE(
    ...
    [
      AC_DEFINE([SC_USE_MEMORY],[1],[define if SC is using <memory>.])
      AC_DEFINE([SC_HAS_STD_SHARED_PTR],[1],[define if std::shared_ptr is available.])
      acx_shared_ptr=yes
    ]
  )
  ...
  if test "$enable_cpptr1" = yes; then
    if test "$acx_shared_ptr" = no; then
      AC_COMPILE_IFELSE(
        ...
        [
          AC_DEFINE([SC_USE_MEMORY],[1],[define if SC is using <memory>.])
          AC_DEFINE([SC_HAS_STD_TR1_SHARED_PTR],[1],[define if std::tr1::shared_ptr is available.])
          acx_shared_ptr=yes
        ]
      )
    fi
    ...
    if test "$acx_shared_ptr" = no; then
      AC_COMPILE_IFELSE(
        ...
        [
          AC_DEFINE([SC_USE_TR1_MEMORY],[1],[define if SC is using <tr1/memory>.])
          AC_DEFINE([SC_HAS_STD_TR1_SHARED_PTR],[1],[define if std::tr1::shared_ptr is available.])
          acx_shared_ptr=yes
        ]
      )
  ...
  if test "$acx_with_boost$acx_shared_ptr" = yesno; then
    AC_DEFINE([SC_USE_BOOST_TR1_MEMORY_HPP],[1],[define if SC is using <boost/tr1/memory.hpp>.])
    AC_DEFINE([SC_HAS_STD_TR1_SHARED_PTR],[1],[define if std::tr1::shared_ptr is available.])
    acx_shared_ptr=yes
  fi
  ...
  AC_COMPILE_IFELSE(
    ...
    [
      AC_DEFINE([SC_HAS_STD_MAKE_SHARED],[1],
...
AC_DEFUN([ACX_CHECK_TYPE_TRAITS], [
  ...
  AC_COMPILE_IFELSE(
    ...
    [
      AC_DEFINE([SC_USE_TYPE_TRAITS],[1],[define if SC is using <type_traits>.])
      AC_DEFINE([SC_HAS_STD_TYPE_TRAITS],[1],[define if std type traits are available.])
      acx_type_traits=yes
    ]
  )
  ...
  if test "$enable_cpptr1" = yes; then
    if test "$acx_type_traits" = no; then
      AC_COMPILE_IFELSE(
        ...
        [
          AC_DEFINE([SC_USE_TYPE_TRAITS],[1],[define if SC is using <type_traits>.])
          AC_DEFINE([SC_HAS_STD_TR1_TYPE_TRAITS],[1],[define if std::tr1 type traits are available.])
          acx_type_traits=yes
        ]
      )
    fi
    ...
    if test "$acx_type_traits" = no; then
      AC_COMPILE_IFELSE(
        ...
        [
          AC_DEFINE([SC_USE_TR1_TYPE_TRAITS],[1],[define if SC is using <tr1/type_traits>.])
          AC_DEFINE([SC_HAS_STD_TR1_TYPE_TRAITS],[1],[define if std::tr1 type traits are available.])
          acx_type_traits=yes
        ]
      )
  ...
  if test "$acx_with_boost$acx_type_traits" = yesno; then
    AC_DEFINE([SC_USE_BOOST_TR1_TYPE_TRAITS_HPP],[1],[define if SC is using <boost/tr1/type_traits.hpp>.])
    AC_DEFINE([SC_HAS_STD_TR1_TYPE_TRAITS],[1],[define if std::tr1 type traits are available.])
    acx_type_traits=yes
  fi
...
AC_DEFUN([ACX_CHECK_ARRAY], [
  ...
  AC_COMPILE_IFELSE(
    ...
    [
      AC_DEFINE([SC_USE_ARRAY],[1],[define if SC is using <array>.])
      AC_DEFINE([SC_HAS_STD_ARRAY],[1],[define if std::array is available.])
      AC_DEFINE([SC_ARRAY_HAS_FILL],[1],[define if array has fill member function.])
      acx_array=yes
    ]
  )
  ...
  if test "$enable_cpptr1" = yes; then
    if test "$acx_array" = no; then
      AC_COMPILE_IFELSE(
        ...
        [
          AC_DEFINE([SC_USE_ARRAY],[1],[define if SC is using <array>.])
          AC_DEFINE([SC_HAS_STD_TR1_ARRAY],[1],[define if std::tr1::array is available.])
    ...
    if test "$acx_array" = no; then
      AC_COMPILE_IFELSE(
        ...
        [
          AC_DEFINE([SC_USE_TR1_ARRAY],[1],[define if SC is using <tr1/array>.])
          AC_DEFINE([SC_HAS_STD_TR1_ARRAY],[1],[define if std::tr1::array is available.])        
          # Check to see if array has fill function
          AC_COMPILE_IFELSE(
            [
  ...
  if test "$acx_with_boost$acx_array" = yesno; then
    AC_DEFINE([SC_USE_BOOST_TR1_ARRAY_HPP],[1],[define if SC is using <boost/tr1/array.hpp>.])
    AC_DEFINE([SC_HAS_STD_TR1_ARRAY],[1],[define if std::tr1::array is available.])
    # Check to see if array has fill function
    AC_COMPILE_IFELSE(
      [
