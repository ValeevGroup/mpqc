 ...
 2  c";
	...
	for ( $i = 0 ; $i < $natom ; $i++ ) {
		printf "%s %14.8f %14.8f %14.8f\n", $qcinp->element($i),
		  $qcinp->position( $i, 0 ), $qcinp->position( $i, 1 ),
		  $qcinp->position( $i, 2 );
	}
...
sub puream {
    ...
    my $val  = $self->{"parser"}->value("puream");
    if ( $val ne "" && $bval eq "" ) {
        $self->error("Bad value for puream: $val");
        $bval = "0";
    }
    elsif ( $val eq "" ) {
        $bval = "1";
...
sub input_string() {
	...
	for ( $i = 0 ; $i < $qcinput->n_atom() ; $i++ ) {
		$mol = sprintf "%s\n    %2s     [ %18.12f %18.12f %18.12f ]", $mol,
		  $qcinput->element($i), $qcinput->position( $i, 0 ),
		  $qcinput->position( $i, 1 ), $qcinput->position( $i, 2 );
	}
	$mol = "$mol\n  }";
	$mol = "$mol\n)\n";
	...
	my $inputmethod = uc( $qcinput->method() );
	my $method      = $methodmap{$inputmethod};
...
sub mpqc_coor {
	...
	my $type = shift @{$coor};
	if ( $type eq "TORS" ) {
		return sprintf "<TorsSimpleCo>:(atoms = [%d %d %d %d])", shift @{$coor},
		  shift @{$coor}, shift @{$coor}, shift @{$coor};
	}
	if ( $type eq "BEND" ) {
		return sprintf "<BendSimpleCo>:(atoms = [%d %d %d])", shift @{$coor},
		  shift @{$coor}, shift @{$coor};
	}
	if ( $type eq "STRE" ) {
		return sprintf "<StreSimpleCo>:(atoms = [%d %d])", shift @{$coor},
		  shift @{$coor};
	}
}
