...
The keywords recognized by \exenm{mpqc} begin with either an \keywd{mpqc}
or \keywd{default} prefix.  That is, they must be nested between an
\keywd{mpqc:(} and a \keywd{)}.  Alternately, each keyword can be prefixed
by \keywd{mpqc:}.  The primary keywords are given below.  Some of the
keywords specify \htmlref{objects}{pkvobject}, in which case the object
will require more \clsnmref{KeyVal} input.  These objects are created from
the input by using their \clsnmref{KeyVal} constructors, which are
documented in
\begin{latexonly}
