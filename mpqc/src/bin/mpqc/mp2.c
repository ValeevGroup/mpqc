...
extern void gcollect(double*, int*, double*);
        ...
        dmt_matrix Scf_Vec, dmt_matrix Fock, FILE* outfile)
{
  ...
  int bf1,bf2,bf3,bf4;
  int nocc=0,nvir;
  int nbasis, nfuncmax, nstatic, mem;
  int nij, ni, nj;
  int nperi, npass, pass;
  int mylena, remaina, firsta;
  int *a2_lengths;
  int *a2_lens;

  double Ecorr=0;

  double *ijab, *intbuf, *evals;
  double *T1, *T2, *T3;

  double_matrix_t scf_vector;
  loop_t *loop;

  tim_enter("init");

  if (me==0) {
    ip_data("base_memory","%d",&bmem,0);
    ip_data("frozen_docc","%d",&nfzc,0);
    ip_data("frozen_uocc","%d",&nfzv,0);
    }
  ...
  if (me==0) {
    if (ip_data("docc","%d",&nocc,1,i) != IPE_OK) {
  ...
  allocbn_double_matrix(&scf_vector,"n1 n2",nbasis,nocc+nvir);
  ...
  */
  ...
  ijab = (double*) malloc(sizeof(double)*nvir*nvir);
  a2_lengths = (int*) malloc(sizeof(int)*nproc);
  a2_lens = (int*) malloc(sizeof(int)*nproc);

  nstatic = (nbasis*(nvir+nocc+1)+nvir*nvir)*sizeof(double);
  mem = bmem-nstatic;

  nj = nocc*nvir*nbasis/nproc;
  nperi = (2*nfuncmax*nfuncmax*nbasis+nj);

  npass = nperi*nocc*sizeof(double)/mem;
 ...
 /* fill the a2_lengths vector for gcollect */
  ...
  T1 = (double*) malloc(sizeof(double)*nfuncmax*nfuncmax*nbasis*ni);
  T2 = (double*) malloc(sizeof(double)*nfuncmax*nfuncmax*nbasis*ni);
  T3 = (double*) malloc(sizeof(double)*nij*nbasis*nvir);
...
if(me==0) printf(" %5d\n",R);
      ...
      for (S=0; S <= R; S++) {
                ...
                for (bf3=0; bf3 < nr ; bf3++) {
                  for (bf4=0; bf4 < ns ;bf4++,index++) {
                    ...
                    if (INT_NONZERO(intbuf[index])) {
                      ...
                      double *c_p = &scf_vector.d[p][firsti];
                      double *c_q = &scf_vector.d[q][firsti];
        ...
        for (a=0; a < mylena; a++) {
          for (q=0; q < nbasis ; q++) {
            double c_qa = scf_vector.d[q][a+firsta+nocc];
          ...
          for (bf4=0; bf4 <= ((R==S) ? bf3 : ns-1) ; bf4++) {
            ...
            for (i=ij=0; i < ni; i++) {
              ...
              for (j=0; j < nocc; j++) {
                ...
                c_rj = scf_vector.d[r][j]*scal;
                c_sj = scf_vector.d[s][j]*scal;
                a3ijs = &T3[ij*nbasis*nvir+s*nvir];
                a3ijr = &T3[ij*nbasis*nvir+r*nvir];
      ...
      for (j=0; j < nocc; j++) {
        ...
        for (s=0; s < nbasis; s++) {
          for (a=0; a < nvir; a++) {
            ...
            double *c_s = &scf_vector.d[s][nocc];
