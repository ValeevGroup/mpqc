     ...
     double_vector_t *_evals, int nfzc, int nfzv, int mem_alloc, FILE* outfile)
{
  ...
  if (me == 0) {
    fprintf(outfile,"Just entered OPT2 program (opt2v2lb)\n");
    }
  ...
  for (j=0; j<nRshell; j++) {
    myshellsizes[j] = INT_SH_NFUNC((centers),myshells[j]);
    split_info[j] = -1;
    }
  ...
  if (maxbf > nbfav) {
    if (me == 0) fprintf(outfile,"Redistributing basis functions\n");
    }
  ...
  while (maxbf > nbfav) {
    ...
    if (me == procmax) {
      ...
      send0(&myshells[shellmaxindex],sizeof(int),2323,procmin);
      send0(&shellmax,sizeof(int),3232,procmin);

      }

    // Send nbfmoved from procmax to all other nodes
    bcast0(&nbfmoved,sizeof(int),mtype_get(),procmax);
    ...
    if (me == procmin) {
      ...
      recv0(&myshells[nRshell-1],sizeof(int),2323);
      recv0(&split_info[nRshell-1],sizeof(int),3232);
  ...
  if (me == 0) {
    fprintf(outfile,"New distribution of basis functions between nodes:\n");
    for (i=0; i<nproc; i++) {
      fprintf(outfile," %4i",nbf[i]);
      if ((i+1)%12 == 0) fprintf(outfile,"\n");
      }
                ...
                for (bf3 = 0; bf3 < nr; bf3++) {
                  ...
                  for (bf4 = 0; bf4 < ns; bf4++) {
                    ...
                    index = bf4 + ns*(bf3+bf3_offset + 
                                      INT_SH_NFUNC((centers),R)*(bf2 + nq*bf1));
      ...
      for (isocc=0; isocc<nsocc; isocc++) {
        ...
        for (i=0; i<nRshell; i++) {
          for (j=0; j<myshellsizes[i]; j++) {
            r = centers->func_num[myshells[i]] + j;
      ...
      for (j=0; j <= (i_offset+i); j++) {
        ...
        for (k=0; k<nRshell; k++) {
          for (l=0; l<myshellsizes[k]; l++) {
            r = centers->func_num[myshells[k]] + l;
