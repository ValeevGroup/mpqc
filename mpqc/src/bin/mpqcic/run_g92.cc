...
int run_g92(char *name_in, const RefKeyVal &g92_keyval, RefMolecule& mole,
            double & energy, RefSCVector& gradient)
{
    // Initialize the list of runtypes and their corresponding names
    static int n_g92_calc_types=5;
    ...
    if (g92_calc[runtype].basis_req)
    {
        ...
        {
            fprintf(stderr,"G92 method requires basis set to be specified in input\n");
            return -1;
        }            
    ...
    run_g92_calc(name, g92_calc[runtype].command, basis, memory,
                use_checkpoint_guess, scratch_dir, g92_dir, mole, charge, multiplicity);
