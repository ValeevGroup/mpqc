...
/*
 *  Componentwise maximum of a vector over all processors,
 *  using given topology.
 *
 *  buf        array of data to be combined by taking maximum.
 ...
 */
{
	...
	gcomb0(buf, items, datatype, msgtype, root, max0) ;
}

#define max(x,y) ((x)>(y)?(x):(y))

void max0 ( p1, p2, items, datatype)
	char *p1, *p2 ;
	int items, datatype ;
{
	char *c1, *c2 ;
