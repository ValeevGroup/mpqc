...
/*
 *  Componentwise minimum of a vector over all processors,
 *  using given topology.
 *
 *  buf        array of data to be combined by taking minimum.
 ...
 */
{
	...
	gcomb0(buf, items, datatype, msgtype, root, min0) ;
}

#define min(x,y) ((x)<(y)?(x):(y))

void min0 ( p1, p2, items, datatype)
	char *p1, *p2 ;
	int items, datatype ;
{
	char *c1, *c2 ;
