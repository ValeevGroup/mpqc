...
char *buf;
{
  ...
  if (envOPN == 1){
    ...
    if (envTOPN == 1){
      ...
      /* do the send */
      ENVSENDBEG(buf, bytes, type, node, envCHECKING);
      ...
      if (envTCP > 1){
        ...
        /* write out info preceding sendbegin */
        ...
        /* write out info following sendbegin */
        envTDATA[envNXT+KEY] = COMPSTATS;
    ...
    else envERRMSG("sendbegin0: communication channel not open - exiting");
