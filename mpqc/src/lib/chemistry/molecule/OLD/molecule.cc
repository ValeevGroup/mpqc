...
DescribedClass_IMPL(Molecule,1,"","")
SavableState_IMPL(Molecule)
void * Molecule::_castdown(const ClassDesc *cd)
{
...
natoms(0)
{
  ...
  int i;
  for (i=0; i<natom; i++) {
      AtomicCenter ac(input.value("atoms",i),
...
{
  fprintf(fp,"Molecule:\n");
  fprintf(fp,"    n  atom  charge         x               y               z"
             "          mass\n");

  int i;
  for (i=0; i<natom(); i++) {
      fprintf(fp," %5d%5s%8.1f%16.10f%16.10f%16.10f%10.5f\n",
              i+1,
	      get_atom(i).element().symbol(),
	      get_atom(i).element().charge(),
	      get_atom(i)[0],
	      get_atom(i)[1],
	      get_atom(i)[2],
...
{
  so.put(natoms);
  if (so.putpointer(atoms)) {
      for (int i=0; i < natoms; i++) so.put(get_atom(i));
    }
}

void Molecule::restore_data_state(int v, StateIn& si)
{
  int natom;
  si.get(natom);
  int ref;
  if ((ref=si.getpointer((void**)&atoms))) {
      natoms=0;
