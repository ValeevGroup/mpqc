...
DescribedClass_IMPL_ABSTRACT(SimpleCo,1,"","")
SavableState_IMPL(SimpleCo)
void * SimpleCo::_castdown(const ClassDesc *cd)
{
...
{
  so.put(value_); so.put(natoms_); so.put(atoms,natoms_); so.putstring(ref);
  }

void SimpleCo::restore_data_state(int version, StateIn& si)
{
  si.get(value_); si.get(natoms_); si.get(atoms); si.getstring(ref);
  }
...
Geom_read_simples(KeyVal *keyval)
{
  ...
  for(int i=0; i < nsimp; i++) {
    const char *val = keyval->value("simp",i,0);
