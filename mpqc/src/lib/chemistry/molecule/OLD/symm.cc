...
DescribedClass_IMPL(SymmCo,1,"","")
SavableState_IMPL(SymmCo)
void *SymmCo::_castdown(const ClassDesc *cd)
{
  ...
  n_simple(n), val(0), label_(0), coeff_(0)
{
  ...
  simple = new SimpleCoPtr[n_simple];
  for (int i=0; i < n_simple; i++) coeff_[i]=1.0;
  }

SymmCo::SymmCo(const char *lab, SimpleCoPtr& sim) : 
  n_simple(1), val(0), label_(0), coeff_(0)
{
  if(lab) { label_ = new char[strlen(lab)+1]; strcpy(label_,lab); }
  coeff_ = new double[1];
  simple = new SimpleCoPtr[1]; simple[0]=sim;
  coeff_[0]=1.0;
  }
  ...
  n_simple(0), val(0), label_(0), coeff_(0)
{
  ...
  for(i=0; i < n_simple; i++) {
    const char *v=kv->Va_value(lab,3,n,1,i);
...
SymmCo& SymmCo::operator=(const SymmCo& s)
{
  ...
  if(simple) delete[] simple; simple=new SimpleCoPtr[n_simple];
...
{
  so.putstring(label_); so.put(n_simple); so.put(val); so.put(coeff_,n_simple);
  if (so.putpointer(simple)) {
    for (int i=0; i < n_simple; i++) so.put(simple[i]);
    }
  }

void SymmCo::restore_data_state(int version, StateIn& si)
{
  si.getstring(label_); si.get(n_simple); si.get(val); si.get(coeff_);
  int ref;
  if ((ref=si.getpointer((void**)&simple))) {
    simple = new SimpleCoPtr[n_simple];
    for (int i=0; i < n_simple; i++) si.get(simple[i]);
    }
  }
