...
\begin{description}
  \item[\keywd{molecule}] A \clsnmref{Molecule} object.  There is no
    default.

  \item[\keywd{coor}] A \clsnmref{MolecularCoor} that describes the
    coordinates.  If this is not given cartesian coordinates will be used.
    For convenience, two keywords needed by the \clsnmref{MolecularCoor}
    object are automatically provided: \keywd{natom3} and
    \keywd{matrixkit}.
...
\begin{description}
  ...
  \item[\keywd{matrixkit}] A \clsnmref{SCMatrixKit} object.  It is
    usually unnecessary to give this keyword.

  \item[\keywd{natom3}] An \clsnmref{SCDimensionObject} for dimension
...
\begin{description}
  \item[\keywd{variable}] Gives a \clsnmref{SetIntCoor} object that
    specifies the internal coordinates that can be varied. If this is not
    given, the variable coordinates will be generated.

  \item[\keywd{followed}] Gives a \clsnmref{SetIntCoor} object that
  ...
  \item[\keywd{fixed}] Gives a \clsnmref{SetIntCoor} object that specifies
  ...
  \item[\keywd{extra\_bonds}] This is only read if the \keywd{generator}
     ...
     bound in extra bond $i$.  The \keywd{extra\_bonds} keyword should only
     be needed for weakly interacting fragments, otherwise all the needed
     bonds will be found.

  \item[\keywd{generator}] Specifies an \clsnmref{IntCoorGen} object that
    ...
    \keywd{extra\_bonds} and this is used to create an
    \clsnmref{IntCoorGen} object.
...
\begin{description}
  ...
  \item[\keywd{linear\_bend\_threshold}] A bend angle greater than 180 -
      \keywd{linear\_tors\_threshold} degrees is considered a linear
      bend. The default is 5.0.

  \item[\keywd{linear\_tors\_threshold}] The angles formed by atoms a-b-c and
      b-c-d are checked for near linearity.  An angle greater than 180 -
      \keywd{linear\_tors\_threshold} degrees the torsion is classified
      as a linear torsion. The default is 5.0.
  ...
  \item[\keywd{extra\_bonds}] This is a vector of atom numbers, where
     elements $2 (i-1) + 1$ and $2 i$ specify the atoms which
     are bound in extra bond $i$.  The \keywd{extra\_bonds} keyword should
...
\begin{description}
  \item[\keywd{generator}] A \clsnmref{IntCoorGen} object that will be
    used to generate the internal coordinates.

  \item[\vrbl{i}] A sequence of integer keywords, all $i$ for 0 $\leq$
    \vrbl{i} $<$ $n$, can be assign to \clsnmref{IntCoor} objects.
...
\begin{description}
  \item[\keywd{mole}] A \clsnmref{MolecularEnergy} object.  If this is not
    given then \keywd{molecule} must be given.

  \item[\keywd{molecule}] A \clsnmref{Molecule} object.  If this is not
    given than \keywd{mole} must be given.
