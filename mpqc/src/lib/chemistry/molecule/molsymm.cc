...
Molecule::highest_point_group(double tol) const
{
    ...
    if (planar) {
      ...
      for (i=0; i<natom(); i++) {
        ...
        for (j=0; j<i; j++) {
          ...
          double norm_perp = perp.norm();
          if (norm_perp < tol) {
            //cout << "  rejected (atoms at same point?)" << endl;
            continue;
            }
          perp *= 1.0/norm_perp;
          if (is_plane(com,perp,tol)) {
  ...
  if (have_c2axis)
    cout << node0 << indent << "  c2axis      = " << c2axis << endl;
  if (have_c2axisperp)
    cout << node0 << indent << "  c2axisperp  = " << c2axisperp << endl;
  if (have_sigmav)
    cout << node0 << indent << "  sigmav      = " << sigmav << endl;
  if (have_sigma)
    cout << node0 << indent << "  sigma       = " << sigma << endl;
#endif
...
#ifdef DEBUG
  cout << "frame:" << endl;
  frame.print(cout << node0);

  cout << "origin:" << endl;
  origin.print(cout << node0);
#endif
