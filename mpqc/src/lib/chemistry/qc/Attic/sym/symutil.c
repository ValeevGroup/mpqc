 ...
 * Initial revision
 *
 * Revision 1.1.1.1  1992/03/17  17:11:21  seidl
 * DOE-NIH Quantum Chemistry Library 0.0
 *
 * Revision 1.1  1992/03/17  17:11:19  seidl
 * Initial revision
 *
...
int _atom;
{
  ...
  _center->atom = (char *) malloc(strlen(at_lab)+1);
  for(i=0; i < strlen(at_lab); i++) _center->atom[i] = 
                                               (char) toupper(at_lab[i]);
...
basis_t *_basis;
{
  ...
  if (errcod = read_basis_(COUNT,atom,basisname,_basis)) return errcod;
