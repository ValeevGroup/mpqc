          ...
          for (long h2b = h1b; h2b < noab;++h2b) {
            for (long h3b = 0L; h3b < noab;++h3b) {
              ...
              if (h2b < h3b) {
                ...
                tileoffset = h2b+noab*(h3b+noab*(h1b+noab*(p6b-noab+nvab*(x5b+(noab+nvab)*h4b))));
              } else if (h3b <= h1b) {
                tileoffset = h2b+noab*(h1b+noab*(h3b+noab*(p6b-noab+nvab*(x5b+(noab+nvab)*h4b))));
              }
                ...
                if (!z->restricted() || z->get_spin(h4b)+z->get_spin(x5b)+z->get_spin(p6b)
                  ...
                  if (z->get_spin(h4b)+z->get_spin(x5b)+z->get_spin(p6b)
                    ...
                    if ((z->get_sym(h4b)^(z->get_sym(x5b)^(z->get_sym(p6b)
                      ...
                      for (long p7b = noab; p7b < noab+nvab; ++p7b) {
                        if (z->get_spin(p6b)+z->get_spin(p7b) == z->get_spin(h1b)+z->get_spin(h2b)) {
                          if ((z->get_sym(p6b)^(z->get_sym(p7b)^(z->get_sym(h1b)^z->get_sym(h2b)))) == z->irrep_t()) {
                            ...
                            long h4b_1,x5b_1,h3b_1,p7b_1;
                            z->restricted_4(h4b,x5b,h3b,p7b,h4b_1,x5b_1,h3b_1,p7b_1);
                            long dim_common = z->get_range(p7b);
                            long dima0_sort = z->get_range(p6b)*z->get_range(h1b)*z->get_range(h2b);
                            long dima1_sort = z->get_range(h4b)*z->get_range(x5b)*z->get_range(h3b);
                            if (p6b < p7b) {
                              z->t2()->get_block(h2b_0+noab*(h1b_0+noab*(p7b_0-noab+nvab*(p6b_0-noab))),k_a0);
                              z->sort_indices4(k_a0, k_a0_sort,
                                  z->get_range(p6b),z->get_range(p7b),
                                  z->get_range(h1b),z->get_range(h2b),
                                  3,2,0,1, +1.0);
                            } else if (p7b <= p6b) {
                              z->t2()->get_block(h2b_0+noab*(h1b_0+noab*(p6b_0-noab+nvab*(p7b_0-noab))),k_a0);
                              z->sort_indices4(k_a0, k_a0_sort,
                                  z->get_range(p7b),z->get_range(p6b),
                                  z->get_range(h1b),z->get_range(h2b),
                                  3,2,1,0, -1.0);
                            }
                            z->vd2_gen()->get_block(p7b_1+z->nab()*(h3b_1+z->nab()*(x5b_1+(noab+nvab)*h4b_1)),k_a1);
                      ...
                      if (h3b >= h2b) {
                        z->sort_indices6(k_c_sort, k_c,
                            z->get_range(h3b),z->get_range(x5b),z->get_range(h4b),
                            z->get_range(h2b),z->get_range(h1b),z->get_range(p6b),
                            2,1,5,4,3,0, -1.0);
                      ...
                      if (h2b >= h3b && h3b >= h1b) {
                        z->sort_indices6(k_c_sort, k_c,
                            ...
                            z->get_range(h2b),z->get_range(h1b),z->get_range(p6b),
                            2,1,5,4,0,3, +1.0);
                        out->add_block(h2b+noab*(h3b+noab*(h1b+noab*(p6b-noab+nvab*(x5b+(noab+nvab)*h4b)))),k_c);
                      }
                      if (h1b >= h3b) {
                        z->sort_indices6(k_c_sort, k_c,
                            z->get_range(h3b),z->get_range(x5b),z->get_range(h4b),
                            z->get_range(h2b),z->get_range(h1b),z->get_range(p6b),
                            2,1,5,0,4,3, -1.0);
                        out->add_block(h2b+noab*(h1b+noab*(h3b+noab*(p6b-noab+nvab*(x5b+(noab+nvab)*h4b)))),k_c);
                      }
                    }
                  }
                }
          ...
          for (long h2b = h1b; h2b < noab;++h2b) {
            for (long h3b = 0L; h3b < noab;++h3b) {
              ...
              if (h2b < h3b) {
                ...
                tileoffset = h2b+noab*(h3b+noab*(h1b+noab*(p6b-noab+nvab*(h5b+noab*h4b))));
              } else if (h3b <= h1b) {
                tileoffset = h2b+noab*(h1b+noab*(h3b+noab*(p6b-noab+nvab*(h5b+noab*h4b))));
              }
              if (out->is_this_local(tileoffset)) {
                if (!z->restricted() || z->get_spin(h4b)+z->get_spin(h5b)+z->get_spin(p6b)
                  ...
                  if (z->get_spin(h4b)+z->get_spin(h5b)+z->get_spin(p6b)
                    ...
                    if ((z->get_sym(h4b)^(z->get_sym(h5b)^(z->get_sym(p6b)
                      ...
                      for (long p7b = noab; p7b < noab+nvab; ++p7b) {
                        if (z->get_spin(p6b)+z->get_spin(p7b) == z->get_spin(h1b)+z->get_spin(h2b)) {
                          if ((z->get_sym(p6b)^(z->get_sym(p7b)^(z->get_sym(h1b)^z->get_sym(h2b)))) == z->irrep_t()) {
                            ...
                            z->vd2()->get_block(p7b_1+z->nab()*(h3b_1+z->nab()*(h5b_1+noab*h4b_1)),k_a1);
                            z->sort_indices4(k_a1, k_a1_sort,
                      ...
                      if (h3b >= h2b) {
                        z->sort_indices6(k_c_sort, k_c,
                            z->get_range(h3b),z->get_range(h5b),z->get_range(h4b),
                            z->get_range(h2b),z->get_range(h1b),z->get_range(p6b),
                            2,1,5,4,3,0, -1.0);
                      ...
                      if (h2b >= h3b && h3b >= h1b) {
                        z->sort_indices6(k_c_sort, k_c,
                            ...
                            z->get_range(h2b),z->get_range(h1b),z->get_range(p6b),
                            2,1,5,4,0,3, +1.0);
                        out->add_block(h2b+noab*(h3b+noab*(h1b+noab*(p6b-noab+nvab*(h5b+noab*h4b)))),k_c);
                      }
                      if (h1b >= h3b) {
                        z->sort_indices6(k_c_sort, k_c,
                            z->get_range(h3b),z->get_range(h5b),z->get_range(h4b),
                            z->get_range(h2b),z->get_range(h1b),z->get_range(p6b),
                            2,1,5,0,4,3, -1.0);
                        out->add_block(h2b+noab*(h1b+noab*(h3b+noab*(p6b-noab+nvab*(h5b+noab*h4b)))),k_c);
                      }
                    }
                  }
                }
