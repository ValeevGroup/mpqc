...
void CCR12_Triples::denom_contraction(){ 
  ...
  double* k_a1_sort= z->mem()->malloc_local_double(triples); 
     ...
     for (long h3b = h2b; h3b < noab; ++h3b) { 
      ...
      for (long h4b = 0L; h4b < noab; ++h4b) { 
       for (long h5b = h4b; h5b < noab; ++h5b) { 
        ...
        // parallelized by the locality of the write opeartion
        const long tileoffset = h3b+noab*(h2b+noab*(h1b+noab*(p6b-noab+nvab*(h5b+noab*h4b)))); 
...
//#define USE_NEW_CODE
#ifdef USE_NEW_CODE 
                z->sort_indices4(k_a0, k_a0_sort, z->get_range(h4b), z->get_range(h5b), z->get_range(h7b), z->get_range(h8b), 0,1,2,3, +1.0); 
#else
...
#ifdef USE_NEW_CODE 
                z->sort_indices6(k_a1, k_a1_sort,
                                 z->get_range(h7b), z->get_range(h8b), z->get_range(p6b),
...
#ifdef USE_NEW_CODE
                z->smith_dgemm(dima1_sort, dima0_sort, dim_common, factor, k_a1_sort, dim_common, k_a0_sort, dim_common, 1.0, k_c_sort, dima1_sort); 
#else
            ...
            z->sort_indices6(k_c_sort, k_c,
                             z->get_range(h4b), z->get_range(h5b), z->get_range(p6b),
                             z->get_range(h1b), z->get_range(h2b), z->get_range(h3b),
                             0,1,2,3,4,5, +1.0); 
#else
            z->sort_indices6(k_c_sort, k_c,
                             z->get_range(h3b), z->get_range(h2b), z->get_range(h1b),
                             z->get_range(p6b), z->get_range(h5b), z->get_range(h4b),
                             5,4,3,2,1,0, +0.5/0.5); 
#endif
