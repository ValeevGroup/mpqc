...
/**************************************************************************
 *
 * This will read in a set of unique atoms from the input, and then create
 * a centers struct around them.  Then a new centers struct will be formed
 * containing all atoms.
 *
 * input:
 *   centers =     pointer to a centers struct, uninitialized
 *   sym_info =    pointer to an uninitialized sym_struct
 *   point_group = string containing schoenflies symbol for the molecular
 *                 point group
 *
 * on return:
 *   centers contains all redundant atoms
 *   sym_info contains all symmetry info
 *
 * returns 0 on success, -1 on failure
 */

GLOBAL_FUNCTION int
sym_init_centers(centers,sym_info,point_group)
centers_t *centers;
...
char *point_group;
{
 ...
 /*
  * count up the unique atoms and then initialize a temp centers struct to
  * hold them
  */

  errcod = ip_count("atoms",&nat,0);
  if (errcod != 0) {
    fprintf(stderr,"sym_init_centers: there don\'t appear to be any atoms\n");
    return(-1);
  }

  i=0; errcod = ip_count("geometry",&i,0);
  if (i != nat) {
    fprintf(stderr,"sym_init_centers: "
                   "size of atoms(%d) != size of geometry(%d)\n",nat,i);
    return(-1);
  }

  errcod = allocbn_centers(&unique_centers,"n",nat);
  if (errcod != 0) {
    fprintf(stderr,"sym_init_centers: could not allocate memory for centers");
    return(-1);
  }

 /* loop over nat, and initialize the unique centers structure */
  for (i=0; i < nat ; i++) {
    center_t *center = &unique_centers.center[i];
    char* atom;
    char* basisset;

    init_center(center);

  /* grab the atom symbol and convert to uppercase */
    errcod = ip_string("atoms",&atom,1,i);
    if (errcod != IPE_OK) {
      fprintf(stderr,"sym_init_centers: no atom %d\n",i);
      return -1;
    }

    for (j=0; j < strlen(atom); j++)
      atom[j] = (char) toupper(atom[j]);

    center->atom = atom;

  /* if there are charges in the input, read them in, otherwise, use the
   * atomic number as the charge
   */
    errcod = ip_data("charges","%lf",&center->charge,1,i);
    if (errcod != IPE_OK) center->charge = atom_to_an(center->atom);
    
    center->r = (double *) malloc(sizeof(double)*3);
    if (!center->r) {
      fprintf(stderr,"sym_init_centers: could not malloc r for atom %d\n",i);
      return -1;
    }
    
  /* now read in the basis set name for this atom.  basis can be an array
   * or a single string
   */
    errcod = ip_count("basis",&j,0);
    if (errcod == IPE_NOT_AN_ARRAY) {
      errcod = ip_string("basis",&basisset,0);
    } else {
      errcod = ip_string("basis",&basisset,1,i);
    }
    
    if (errcod != IPE_OK) {
      fprintf(stderr,"sym_init_centers: could read basis for atom %d\n",i);
      return -1;
    }

    errcod = int_read_basis(sym_to_atom(center->atom),basisset,&center->basis);
    if (errcod != IPE_OK) {
      fprintf(stderr,"sym_init_centers: "
                     "could not form basis set for atom %d\n",i);
      return -1;
    }

    for (j=0; j < 3 ; j++) {
      errcod = ip_data("geometry","%lf",&center->r[j],2,i,j);
      if(errcod != 0) {
        fprintf(stderr,"sym_init_centers: "
                       "problem reading the geometry (%d,%d)",i,j);
        return(-1);
      }
    }
  }

  errcod = sym_init_given_centers(&unique_centers,centers,sym_info,point_group);
