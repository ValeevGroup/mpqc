...
Gaussian92::parse_g92_gradient()
{
  ...
  RefSCVector gradient(moldim(),matrixkit());
...
Gaussian92::parse_g92_hessian()
{
  ...
  RefSymmSCMatrix hessian(moldim(),matrixkit());
...
Gaussian92::run_g92(const char *method)
{
  ...
  for (int i=0; i < molecule()->natom(); i++) {
    fprintf(g92com,"%s %lf %lf %lf\n",
            ...
            molecule()->atom(i).operator[](0), molecule()->atom(i).operator[](1),
            molecule()->atom(i).operator[](2));
  }
  fprintf(g92com,"\n");
  fclose(g92com);
...
RefDiagSCMatrix
Gaussian92::eigenvalues()
{
  cerr << "Gaussian92::eigenvalues: not available" << endl;
  abort();
  return 0;
}

double
Gaussian92::occupation(int irrep, int vectornum)
{
  cerr << "Gaussian92::occupation: not available" << endl;
  abort();
  return 0.0;
}

void
Gaussian92::print(ostream&o)
{
  o.flush();
...
{
  if (!hessian_.computed())
    run_hessian();

  RefSCDimension nnorm = new SCDimension(moldim()->n()-6);
  RefSCMatrix normalmodes(moldim(),nnorm,matrixkit());
...
Gaussian92::frequencies()
{
  ...
  RefSCDimension nfreq = new SCDimension(moldim()->n()-6);
  RefSCVector freq(nfreq,matrixkit());
  freq.assign(0.0);
