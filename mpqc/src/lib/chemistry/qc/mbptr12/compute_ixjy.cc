...
TwoBodyMOIntsTransform_ixjy::compute()
{
  ...
  */
  ...
  for (int pass=0; pass<npass_; pass++) {
    ...
    bzerofast(integral_ijsx, (num_te_types_*nij*ints_acc_->blocksize()));
    integral_ijsx = 0;
    mem_->sync();
    ExEnv::out0() << indent
...
#   endif
        ...
        for (int i = 0; i<ni; i++) {
          for (int x = 0; x<rank2; x++) {
            for (int j = 0; j<rank3; j++) {
              ...
              if (ij%nproc == me) {
                const double* ijsx_ints = integral_ijsx + (ij_local*num_te_types_+te_type)*ints_acc_->blocksize();
    ...
    size_t blksize = rank2*rank4*sizeof(double);
      ...
      for (int i = 0; i<ni; i++) {
        for (int j = 0; j<rank3; j++) {
          ...
          if (ij%nproc == me) {
            ...
            const double *sx_ptr = integral_ijsx + (ij_local*num_te_types_+te_type)*ints_acc_->blocksize();
            ...
            memcpy((void*)sx_ptr,(const void*)ijxy_ints,blksize);
          }
        }
      }
    }
        ...
        for (int i = 0; i<ni; i++) {
          for (int x = 0; x<rank2; x++) {
            for (int j = 0; j<rank3; j++) {
              ...
              int ij_local = ij/nproc;
              if (ij%nproc == me) {
                const double* ijxy_ints = integral_ijxy + (ij_local*num_te_types_+te_type)*ints_acc_->blocksize();
                for (int y = 0; y<rank4; y++) {
                  double value = ijxy_ints[x*rank4+y];
                  printf("4Q: type = %d (%d %d|%d %d) = %12.8f\n",
                         te_type,i+restart_orbital+2,x,j+2,y,value);
                }
              }
            }
          }
    ...
    ints_acc_->store_memorygrp(mem_,ni);
