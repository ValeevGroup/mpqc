 ...
 */
	...
	class Slater1D {
	public:
	    Slater1D(double a, int k=0) : a_(a), k_(k) {}
	    double operator()(double x) const { return std::pow(x,k_) * std::exp(-a_*x); }
	private:
	    int k_;
	    double a_;
	};
	...
	class Gaussian1D {
	public:
	    Gaussian1D(double a, int k=0) : a_(a), k_(k) {}
	    double operator()(double x) const { return std::pow(x,k_) * std::exp(-a_*x*x); }
	private:
	    int k_;
	    double a_;
	};
	...
	public:
	    PowerGaussian1D(double a, int l=2, int k=0) : a_(a), k_(k), l_(l) {}
	    double operator()(double x) const { return std::pow(x,k_) * std::exp(-a_*std::pow(x,l_)); }
	private:
	    int k_;
	    int l_;
	    double a_;
