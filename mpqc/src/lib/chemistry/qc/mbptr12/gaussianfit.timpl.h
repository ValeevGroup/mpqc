...
extern "C" {
    void __eval_slater(double* params, double* f, int nparam, int np, void *extraparams);
    void __eval_slater_dfdp(double* params, double* dfdp, int nparam, int np, void *extraparams);
    void __eval_slater_pgauss(double* params, double* f, int nparam, int np, void *extraparams);
    void __eval_slater_dfdp_pgauss(double* params, double* dfdp, int nparam, int np, void *extraparams);
};

namespace {
    extern "C" {
	typedef void (*eval_f_ptr)(double *p, double *hx, int m, int n, void *adata);
	typedef void (*eval_dfdp_ptr)(double *p, double *hx, int m, int n, void *adata);
    };
}
...
namespace sc {
    ...
    template <typename Function,
	      typename Weight>
    GaussianFit<Function,Weight>::GaussianFit(unsigned int N,
					      const Weight& W,
					      double left,
					      double right,
					      unsigned int NP) :
	weight_(W), p_(new double[2*N]), scratch_(new double[NP]), npts_(NP), left_(left), right_(right)
    {
    ...
    template <typename Function,
	      typename Weight>
    GaussianFit<Function,Weight>::~GaussianFit() {
	delete[] p_;
	delete[] scratch_;
    }
    ...
    GaussianFit<Function,Weight>::operator()(const Function& F) const
    {
