//
// r12a_intermed_spinorb_sbs_2.cc
//
...
/*-------------------------------------
 ...
 -------------------------------------*/
void
MBPT2_R12::compute_r12a_intermed_spinorb_SBS_2_()
{
  // Basis Sets
  ...
  // Keep Scf_Vec but get rid of evalmat
  evalmat = 0;
  ...
  for (i=0; i<thr_->nthread(); i++) {
    e12thread[i] = new R12A_GRT_12Qtr(i, thr_->nthread(), me, nproc,
				      mem, msg_, lock, basis(), basis(), tbints_[i],
...
#   endif
  ...
  Ref<OneBodyInt> m2_ints = integral()->quadrupole(0);
  Ref<PetiteList> pl = integral()->petite_list();
  // form skeleton dipole moment matrices
  RefSymmSCMatrix mx(bs->basisdim(), bs->matrixkit());
  RefSymmSCMatrix my(bs->basisdim(), bs->matrixkit());
  RefSymmSCMatrix mz(bs->basisdim(), bs->matrixkit());
  RefSymmSCMatrix mxx(bs->basisdim(), bs->matrixkit());
  RefSymmSCMatrix myy(bs->basisdim(), bs->matrixkit());
  RefSymmSCMatrix mzz(bs->basisdim(), bs->matrixkit());
  ...
  for(int sh1=0; sh1<nshell; sh1++) {
    ...
    for(int sh2=0; sh2<=sh1; sh2++) {
      ...
      int deg = pl->in_p2(sh1,sh2);
      if (deg == 0)
	continue;
      double symm_pfac = (double) deg;
      ...
      for(int bf1=0; bf1<nbf1; bf1++, bf1_index++) {
	int bf2_index = bf2_offset;
	for(int bf2=0; bf2<nbf2; bf2++, bf2_index++) {
	  mx.set_element(bf1_index, bf2_index, symm_pfac * *(m1intsptr++));
	  my.set_element(bf1_index, bf2_index, symm_pfac * *(m1intsptr++));
	  mz.set_element(bf1_index, bf2_index, symm_pfac * *(m1intsptr++));
	  mxx.set_element(bf1_index, bf2_index, symm_pfac * *(m2intsptr++));
          m2intsptr += 2;
	  myy.set_element(bf1_index, bf2_index, symm_pfac * *(m2intsptr++));
	  m2intsptr++;
	  mzz.set_element(bf1_index, bf2_index, symm_pfac * *(m2intsptr++));
	}
      }
    }
  }
  // then symmetrize it
  RefSCDimension sodim = pl->SO_basisdim();
  RefSymmSCMatrix mx_so(sodim, bs->so_matrixkit());
  RefSymmSCMatrix my_so(sodim, bs->so_matrixkit());
  RefSymmSCMatrix mz_so(sodim, bs->so_matrixkit());
  RefSymmSCMatrix mxx_so(sodim, bs->so_matrixkit());
  RefSymmSCMatrix myy_so(sodim, bs->so_matrixkit());
  RefSymmSCMatrix mzz_so(sodim, bs->so_matrixkit());
  pl->symmetrize(mx,mx_so);
  pl->symmetrize(my,my_so);
  pl->symmetrize(mz,mz_so);
  pl->symmetrize(mxx,mxx_so);
  pl->symmetrize(myy,myy_so);
  pl->symmetrize(mzz,mzz_so);
  // and clean up a bit
  m1_ints = 0;
  pl = 0;
  mx = 0;
  ...
  MX = Scf_Vec * mx_so * Scf_Vec.t();
  MY = Scf_Vec * my_so * Scf_Vec.t();
  MZ = Scf_Vec * mz_so * Scf_Vec.t();
  MXX = Scf_Vec * mxx_so * Scf_Vec.t();
  MYY = Scf_Vec * myy_so * Scf_Vec.t();
  MZZ = Scf_Vec * mzz_so * Scf_Vec.t();
  }
  Scf_Vec = 0;
  ...
  if (r12intsacc_->has_access(me)) {
	...
	int lk_ab = l*nocc_act + k;
        ...
        double *klyx_buf_eri, *klyx_buf_r12, *klyx_buf_r12t1;
        klyx_buf_eri = r12intsacc_->retrieve_pair_block(k,l,R12IntsAcc::eri);
        klyx_buf_r12 = r12intsacc_->retrieve_pair_block(k,l,R12IntsAcc::r12);
	...
	int ij = 0;
	    ...
	    /*----------------------------------
	     ...
	     ----------------------------------*/
	    double r1r1_ik = MXX->get_element(i,k) + MYY->get_element(i,k) + MZZ->get_element(i,k);
	    double r1r1_il = MXX->get_element(i,l) + MYY->get_element(i,l) + MZZ->get_element(i,l);
	    double r1r1_jk = MXX->get_element(j,k) + MYY->get_element(j,k) + MZZ->get_element(j,k);
	    double r1r1_jl = MXX->get_element(j,l) + MYY->get_element(j,l) + MZZ->get_element(j,l);
