...
namespace sc {
  ...
  const Ref<MOIndexSpace>&PsiCorrWavefunction::vir_act_sb(SpinCase1 spin) {
  ...
  static ClassDesc PsiCCSD_PT2R12_cd(typeid(PsiCCSD_PT2R12), "PsiCCSD_PT2R12",
                                     1, "public PsiCC", 0, create<
                                         PsiCCSD_PT2R12>,
  ...
  void PsiCCSD_PT2R12::compute() {
    ...
    if (use_sparsemap) {
      ...
      RefSCMatrix MPQC2PSI_tform_va = transform(*vir_act_sb_psi, *vir_act,
                                                localkit);
      T1[AlphaBeta] = transform_T1(MPQC2PSI_tform_oa, MPQC2PSI_tform_va,
                                   T1_psi, localkit);
    ...
    // Make H1_R0[AlphaAlpha]
    {
      ...
      antisymmetrize(H1_R0[AlphaAlpha], H1_R0[AlphaBeta],
                     r12eval->xspace(Alpha), r12eval->occ_act(Alpha));
    }
