  ...
  for (int i=0; i < vec->nrow(); i++) {
    for (int j=0; j <= i; j++) {
      ...
      for (int k=ndocc+1; k < ndocc+2; k++)
        pob += vec->get_element(i,k)*vec->get_element(j,k);
...
OSSSCF::do_gradient(const RefSCVector& gradient)
{
  ...
  alpha[0][0] = 1.0;
  alpha[1][0] = alpha[0][1] = alpha[2][0] = alpha[0][2] = 0.5;
  alpha[1][1] = alpha[2][1] = alpha[1][2] = alpha[2][2] = 0.25;

  memset(beta,0,sizeof(double)*16);

  beta[0][0] = -1.0;
  beta[1][0] = beta[2][0] = beta[0][1] = beta[0][2] = beta[1][1] =
    beta[2][2] = -0.5;
  beta[2][1] = beta[1][2] = 0.5;
  ...
  // form density
  double pmax;
  gr_density(_gr_vector,_gr_dens,_gr_opa_dens,_gr_opb_dens,_ndocc,pmax);

  mofock.assign(_gr_opa_dens);
  mofock.accumulate(_gr_opb_dens);
  mofock.scale(0.5);
                    ...
                    for (int lp=0; lp < sz[3]; lp++) {
                      ...
                      contrib = alpha[0][0] * _gr_dens.get_element(io,jo)*
                              ...
                              + alpha[1][1] * _gr_opa_dens.get_element(io,jo)*
                                              _gr_opa_dens.get_element(ko,lo)
                              + alpha[2][1] * _gr_opb_dens.get_element(io,jo)*
                                              _gr_opa_dens.get_element(ko,lo)
                              + alpha[0][2] * _gr_dens.get_element(io,jo)*
                                              _gr_opb_dens.get_element(ko,lo)
                              + alpha[1][2] * _gr_opa_dens.get_element(io,jo)*
                                              _gr_opb_dens.get_element(ko,lo)
                      ...
                      contrib = beta[0][0] * _gr_dens.get_element(io,ko)*
                              ...
                              + beta[1][1] * _gr_opa_dens.get_element(io,ko)*
                                             _gr_opa_dens.get_element(jo,lo)
                              ...
                              + beta[1][2] * _gr_opa_dens.get_element(io,ko)*
                                             _gr_opb_dens.get_element(jo,lo)
                              + beta[2][2] * _gr_opb_dens.get_element(io,ko)*
                                             _gr_opb_dens.get_element(jo,lo);
                      contrib *= contmp;
                      ...
                      if (i!=j && k!=l) {
                        ...
                        contrib = beta[0][0] * _gr_dens.get_element(io,lo)*
                                ...
                                + beta[1][1] * _gr_opa_dens.get_element(io,lo)*
                                               _gr_opa_dens.get_element(jo,ko)
                                ...
                                + beta[1][2] * _gr_opa_dens.get_element(io,lo)*
                                               _gr_opb_dens.get_element(jo,ko)
                                + beta[2][2] * _gr_opb_dens.get_element(io,lo)*
                                               _gr_opb_dens.get_element(jo,ko);
                        contrib *= contmp;
