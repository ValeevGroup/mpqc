...
void
TCSCF::form_density(const RefSCMatrix& vec,
    ...
    for (int i=0; i < nbasis; i++) {
      for (int j=0; j <= i; j++) {
        ...
        for (int k=_ndocc; k < _ndocc+1; k++)
          ptoa += occa*lvec->get_element(i,k)*lvec->get_element(j,k);
        ...
        for (int k=_ndocc+1; k < _ndocc+2; k++)
          ptob += occb*lvec->get_element(i,k)*lvec->get_element(j,k);
...
void
TCSCF::form_ao_fock(centers_t *centers, double *intbuf)
{
    ...
    for (int i=0; i < nbasis; i++) {
      for (int j=0; j <= i; j++) {
        ...
        for (int k=ndocc; k < ndocc+1; k++)
          pto1 += 2.0*lvec->get_element(i,k)*lvec->get_element(j,k);

        double pto2=0;
        for (int k=ndocc+1; k < ndocc+2; k++)
          pto2 += 2.0*lvec->get_element(i,k)*lvec->get_element(j,k);

        ldens1->set_element(i,j,pt+pto1);
        ldens2->set_element(i,j,pt+pto2);
      }
    }
  }
}
...
void
TCSCF::new_coeff(centers_t *centers, double *intbuf)
{
  // form density where op1_dens is closed shell density with open(1) doubly
  // occupied, and op2_dens is closed shell density with open(2) doubly
  // occupied
  tcdens(_gr_vector,_gr_op1_dens,_gr_op2_dens,_ndocc,basis()->nbasis());

  _gr_vector.print("vector");
  _gr_op1_dens.print("first density");
  _gr_op2_dens.print("second density");
  
  RefSymmSCMatrix d1 = _gr_op1_dens.copy();
  RefSymmSCMatrix d2 = _gr_op2_dens.copy();

  d1.accumulate(d2);
  d2.scale(-1.0);
  d2.accumulate(_gr_op1_dens);

  _gr_op1_dens.assign(d1);
  _gr_op2_dens.assign(d2);
  
  d1.assign(0.0);
  d2.assign(0.0);
  
  _gr_op1_dens->scale(2.0);
  _gr_op1_dens->scale_diagonal(0.5);
  _gr_op2_dens->scale(2.0);
  _gr_op2_dens->scale_diagonal(0.5);
                    ...
                    if (INT_NONZERO(intbuf[index])) {
                      ...
                      if (jj == kk) {
                        ...
                        if (ii == jj || kk == ll) {
                          ...
                          d1.accumulate_element(ii,jj,pkval*
                                             _gr_op1_dens.get_element(kk,ll));
                          d1.accumulate_element(kk,ll,pkval*
                                             _gr_op1_dens.get_element(ii,jj));

                          d2.accumulate_element(ii,jj,pkval*
                                             _gr_op2_dens.get_element(kk,ll));
                          d2.accumulate_element(kk,ll,pkval*
                                             _gr_op2_dens.get_element(ii,jj));
                          ...
                          d1.accumulate_element(ii,ll,-pkval*
                                             _gr_op1_dens.get_element(kk,jj));
                          d1.accumulate_element(kk,jj,-pkval*
                                             _gr_op1_dens.get_element(ii,ll));

                          d2.accumulate_element(ii,ll,-pkval*
                                             _gr_op2_dens.get_element(kk,jj));
                          d2.accumulate_element(kk,jj,-pkval*
                                             _gr_op2_dens.get_element(ii,ll));
                        ...
                        d1.accumulate_element(ii,kk,-pkval*
                                             _gr_op1_dens.get_element(jj,ll));
                        d1.accumulate_element(jj,ll,-pkval*
                                             _gr_op1_dens.get_element(ii,kk));

                        d2.accumulate_element(ii,kk,-pkval*
                                             _gr_op2_dens.get_element(jj,ll));
                        d2.accumulate_element(jj,ll,-pkval*
                                             _gr_op2_dens.get_element(ii,kk));
  ...
  for (int i=0; i < d1.n(); i++) {
    for (int j=0; j <= i; j++) {
      eci1 += _gr_op2_dens.get_element(i,j)*_gr_hcore.get_element(i,j);
      eci2 += _gr_op2_dens.get_element(i,j)*d1.get_element(i,j);
      eci3 += _gr_op2_dens.get_element(i,j)*d2.get_element(i,j);
    }
  }

  double term = 0.5*(eci1+0.5*eci2);
  double hoff = -0.25*eci3;
  double amax = term*term + hoff*hoff;
  amax = sqrt(amax);

  if (term < 0.0)
    amax = -amax;

  double cn = (amax+term)/(2*amax);
  cn = sqrt(cn);
  double sn = hoff/(cn*(amax+amax));
