...
\begin{description}
  \item[\keywd{matrixkit}] Gives a \clsnmref{SCMatrixKit}
    \htmlref{object}{pkvobject}.  If it is not specified, a default
    \clsnmref{SCMatrixKit} is selected.
...
\begin{description}
  ...
  \item[\keywd{function}] A \clsnmref{Function}
     \htmlref{object}{pkvobject}.  There is no default.

  \item[\keywd{convergence}] This can be either a floating point number or
     a \clsnmref{Convergence} \htmlref{object}{pkvobject}.  If it is a
     floating point number then it is the convergence criterion.  See the
     description \clsnmref{Convergence} class for the default.
...
\begin{description}
  \item[\keywd{update}] This gives a \clsnmref{HessianUpdate}
     \htmlref{object}{pkvobject}.  The default is to not update the
     hessian.

  \item[\keywd{hessian}] By default, the guess hessian is obtained from the
     \clsnmref{Function} \htmlref{object}{pkvobject}.  This keyword
     specifies an lower triangle \htmlref{array}{pkvarray} (the second
  ...
  \item[\keywd{lineopt}] This gives a \clsnmref{LineOpt}
     \htmlref{object}{pkvobject} for doing line optimizations in the Newton
...
\begin{description}
  \item[\keywd{update}] This gives an \clsnmref{HessianUpdate}
     \htmlref{object}{pkvobject}.  The default is to not update the
     hessian.
  ...
  \item[\keywd{mode\_following}] If this is true, then the initial search
     direction for a transition state search will be choosen to similar to
     the first coordinate of the \clsnmref{Function}.  The default is
     false.
...
\begin{description}
  ...
  \item[\keywd{ngroup}] The number of iterations in a DIIS group.  DIIS
     extrapolation is only used for the first \keywd{ngroupdiis} of these
     interations.  The default is 1.  If \keywd{ngroup} is 1 and
     \keywd{ngroupdiis} is greater than 0, then DIIS will be used on all
     iterations after and including the \keywd{start} iteration.

  \item[\keywd{ngroupdiis}] The number of DIIS extrapolations to do at the
