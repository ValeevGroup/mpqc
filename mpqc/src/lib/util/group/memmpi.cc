...
MPIMemoryGrp::get_mid()
{
  for (int i=0; i<max_mid; i++) {
      if (mid_ready_[i]) {
          mid_ready_[i] = 0;
          if (debug_) ExEnv::out() << "MPIMemoryGrp::get_mid(): got " << i << endl;
          return i;
        }
    }
...
{
  if (debug_) ExEnv::out() << "MPIMemoryGrp::free_mid(): freeing " << mid << endl;
  mid_ready_[mid] = 1;
}
...
MPIMemoryGrp::send(void* data, int nbytes, int node, int type)
{
  ...
  if (debug_) ExEnv::out() << ">>>> MPI_Isend for mid " << mid
                   << " type " << type << endl;
  int ret;
...
MPIMemoryGrp::recv(void* data, int nbytes, int node, int type)
{
  ...
  if (debug_) ExEnv::out() << ">>>> MPI_Irecv for mid " << mid
                   << " type " << type << endl;
  int ret;
...
MPIMemoryGrp::wait(long mid1, long mid2)
{
  ...
  MPI_Status status;
  if (debug_) {
      ExEnv::out() << me() << ": MPIMemoryGrp::wait("
           << mid1 << ", "
           << mid2 << ")"
           << endl;
    }
  ...
  if (mid1 == -1) {
      ...
      free_mid(mid1);
      if (debug_)
          ExEnv::out() << me() << ": MPIMemoryGrp::wait(): got(1) " << mid1 << endl;
      return mid1;
    }
  else {
      while(1) {
          ...
          if (flag) {
              free_mid(mid1);
              if (debug_)
                  ExEnv::out() << me() << ": MPIMemoryGrp::wait(): got(2a) "
                       << mid1 << endl;
              return mid1;
            }
          ...
          if (flag) {
              free_mid(mid2);
              if (debug_)
                  ExEnv::out() << me() << ": MPIMemoryGrp::wait(): got(2b) "
                       << mid2 << endl;
              return mid2;
            }
        }
...
MPIMemoryGrp::probe(long mid)
{
  ...
  if (flag) {
      free_mid(mid);
      if (debug_)
          ExEnv::out() << me() << ": MPIMemoryGrp::probe(): got "
               << mid << endl;
      return 1;
    }
  return 0;
...
MPIMemoryGrp::deactivate()
{
  if (active_) {
      if (debug_)
          ExEnv::out() << "MPIMemoryGrp::deactivate()" << endl;
      sync();
      if (debug_)
          ExEnv::out() << ">>>> MPI_Cancel for " << data_request_mid_ << endl;
      int ret;
#ifndef __PUMAGON__
