...
class GaussianBasisSet: virtual public SavableState, virtual public DescribedXMLWritable
{
  ...
  public:
    ...
    /// Intended for use with C++11 range-style iterators
    const GaussianBasisSet::shell_iter_wrapper shell_iterator(
        const Ref<GaussianBasisSet>& dfbasis = 0
    );
    ...
    /// Intended for use with C++11 range-style iterators
    const GaussianBasisSet::function_iter_wrapper function_iterator(
        const Ref<GaussianBasisSet>& dfbasis = 0
    );
    ...
    const GaussianBasisSet::shell_iter_wrapper iter_shells_on_center(
        int center,
        const Ref<GaussianBasisSet>& dfbasis = 0
    );

    const GaussianBasisSet::function_iter_wrapper iter_functions_on_center(
        int center,
        const Ref<GaussianBasisSet>& dfbasis = 0
    );

    const GaussianBasisSet::function_iter_wrapper iter_functions_in_shell(
        int shellnum,
        const Ref<GaussianBasisSet>& dfbasis = 0
    );


    const ShellData shell_data(int ish, const Ref<GaussianBasisSet>& dfbasis = 0);
    const BasisFunctionData function_data(int ibf, const Ref<GaussianBasisSet>& dfbasis = 0);
...
// TODO change member variables to methods in these?
/// The object that shell iterator dereferences to.
class ShellData {
  ...
  public:
    ...
    friend class ShellIter;
    friend class GaussianBasisSet;
};

/// The object that basis function iterator dereferences to.
class BasisFunctionData {
  ...
  public:
    ...
    friend class BasisFunctionIter;
    friend class GaussianBasisSet;
};
