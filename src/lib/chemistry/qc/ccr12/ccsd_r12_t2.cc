...
smith_2_38(); //z->t1(),z->v2()=>in.at(2)
smith_1_5(); //z->t2(),in.at(2)=>in.at(1)
delete in.at(2);
in.at(2)=new Tensor("ccsd_r12_t2_2_2",z->mem());
offset_smith_1_10();
smith_1_10_0(); //z->v2()=>in.at(2)
in.at(3)=new Tensor("ccsd_r12_t2_3_0",z->mem());
offset_smith_2_18();
smith_2_18_0(); //z->v2()=>in.at(3)
smith_3_30(); //z->t1(),z->v2()=>in.at(3)
smith_2_18(); //z->t1(),in.at(3)=>in.at(2)
delete in.at(3);
smith_2_33(); //z->gt2(),z->vr2()=>in.at(2)
smith_2_34(); //z->t2(),z->v2()=>in.at(2)
...
smith_1_46(); //z->gt2(),z->vr2()=>in.at(1)
...
void CCSD_R12_T2::offset_smith_1_26(){ 
     ...
     if (!z->restricted() || z->get_spin(h6b)+z->get_spin(p4b)+z->get_spin(h2b)+z->get_spin(q5b)!=8L) { 
      if (z->get_spin(h6b)+z->get_spin(p4b)==z->get_spin(h2b)+z->get_spin(q5b)) { 
       if ((z->get_sym(h6b)^(z->get_sym(p4b)^(z->get_sym(h2b)^z->get_sym(q5b))))==(z->irrep_e()^z->irrep_v())) { 
           ...
           if ((z->get_sym(p4b)^(z->get_sym(q7b)^(z->get_sym(h2b)^z->get_sym(h8b))))==z->irrep_e()) { 
            ...
            if (dima0>0L && dima1>0L) { 
             ...
             if (h6b<h8b && q5b<q7b) { 
              ...
              z->sort_indices4(k_a1,k_a1_sort,z->get_range(h6b),z->get_range(h8b),z->get_range(q5b),z->get_range(q7b),2,0,3,1,+1.0); 
             } 
             else if (h6b<h8b && q7b<=q5b) { 
              z->v2()->get_block(q5b_1+(z->nab())*(q7b_1+(z->nab())*(h8b_1+(z->nab())*(h6b_1))),k_a1); 
              z->sort_indices4(k_a1,k_a1_sort,z->get_range(h6b),z->get_range(h8b),z->get_range(q7b),z->get_range(q5b),3,0,2,1,-1.0); 
             } 
             else if (h8b<=h6b && q5b<q7b) { 
              z->v2()->get_block(q7b_1+(z->nab())*(q5b_1+(z->nab())*(h6b_1+(z->nab())*(h8b_1))),k_a1); 
              z->sort_indices4(k_a1,k_a1_sort,z->get_range(h8b),z->get_range(h6b),z->get_range(q5b),z->get_range(q7b),2,1,3,0,-1.0); 
             } 
             else if (h8b<=h6b && q7b<=q5b) { 
              z->v2()->get_block(q5b_1+(z->nab())*(q7b_1+(z->nab())*(h6b_1+(z->nab())*(h8b_1))),k_a1); 
              z->sort_indices4(k_a1,k_a1_sort,z->get_range(h8b),z->get_range(h6b),z->get_range(q7b),z->get_range(q5b),3,1,2,0,+1.0); 
             } 
             z->mem()->free_local_double(k_a1); 
             double factor=1.0; 
             z->smith_dgemm(dima0_sort,dima1_sort,dim_common,factor,k_a0_sort,dim_common,k_a1_sort,dim_common,1.0,k_c_sort,dima0_sort); 
        ...
        double* k_c=z->mem()->malloc_local_double(dimc); 
        z->sort_indices4(k_c_sort,k_c,z->get_range(q5b),z->get_range(h6b),z->get_range(h2b),z->get_range(p4b),1,3,2,0,+0.5); 
        in[1]->add_block(q5b-z->noab()-z->nvab()+z->ncab()*(h2b+z->noab()*(p4b-z->noab()+z->nvab()*(h6b))),k_c); 
        z->mem()->free_local_double(k_c); 
        z->mem()->free_local_double(k_c_sort); 
       } 
          ...
          if ((z->get_sym(p4b)^(z->get_sym(q6b)^(z->get_sym(h7b)^z->get_sym(h8b))))==z->irrep_e()) { 
           ...
           if (dima0>0L && dima1>0L) { 
            ...
            if (q5b<q6b) { 
             ...
             z->sort_indices4(k_a1,k_a1_sort,z->get_range(h7b),z->get_range(h8b),z->get_range(q5b),z->get_range(q6b),2,3,1,0,+1.0); 
            } 
            else if (q6b<=q5b) { 
             z->v2()->get_block(q5b_1+(z->nab())*(q6b_1+(z->nab())*(h8b_1+(z->nab())*(h7b_1))),k_a1); 
             z->sort_indices4(k_a1,k_a1_sort,z->get_range(h7b),z->get_range(h8b),z->get_range(q6b),z->get_range(q5b),3,2,1,0,-1.0); 
            } 
            z->mem()->free_local_double(k_a1); 
            double factor=1.0; 
            if (h7b==h8b) { 
    ...
    if (z->get_spin(h5b)==z->get_spin(q6b)) { 
     if ((z->get_sym(h5b)^z->get_sym(q6b))==z->irrep_f()) { 
      ...
      if (dima0>0L) { 
       ...
       z->sort_indices2(k_a0,k_a0_sort,z->get_range(h5b),z->get_range(q6b),0,1,+1.0); 
       z->mem()->free_local_double(k_a0); 
       double* k_c=z->mem()->malloc_local_double(dimc); 
       z->sort_indices2(k_a0_sort,k_c,z->get_range(h5b),z->get_range(q6b),0,1,+1.0); 
...
void CCSD_R12_T2::offset_smith_2_18(){ 
