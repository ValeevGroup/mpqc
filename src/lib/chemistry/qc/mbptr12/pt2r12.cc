...
# define DEBUGG false
  ...
  if (use_mpqc3_) {
    ...
    typedef SingleReference_R12Intermediates<double>::TArray4d TArray4;
    typedef SingleReference_R12Intermediates<double>::TArray2 TArray2;

    bootup_mpqc3();

    // make all the matrixes that needed
    TArray2 gamma2 = __2("<m|gamma|n>");
    TArray4 gamma4 = __4("<m n|gamma|m1 n1>");

    TArray4 g = __4("<n1 m1|g|m n>");

    TArray2 h = __2("<m|h|n>");

    TArray2 Fmn = __2("<m|F|n>");
    TArray2 FmA = __2("<m|F|A'>");
    TArray2 Fcn = __2("<c'|F|n>");
    TArray2 FAB = __2("<A'|F|B'>");

    TArray2 IAB = __2("<B'|I|A'>");
    TArray2 IBc = __2("<B'|I|c'>");

    // make B matrix in Equation (26)
    // term1
    TArray4 term1; term1("B',A',y,x") = FAB("B',A'") * gamma2("y,x");
#if DEBUGG
...
#endif
    // term2
    TArray4 term2; term2("B',A',y,x") = - IAB("B',A'") * (g("y,k,i,j") * gamma4("i,j,x,k"));
    TArray4 term12; term12("B',A',y,x") = term1("B',A',y,x") + term2("B',A',y,x");
#if DEBUGG
    std::cout << "term12 of B: \n" << term12 << std::endl;
#endif
    // term3
    TArray4 term3; term3("B',A',y,x") = - IAB("B',A'") * (h("y,i") * gamma2("i,x"));
    // sum 3 terms
    TArray4 B; B("B',A',y,x") = term12("B',A',y,x") + term3("B',A',y,x");
#if DEBUGG
    ...
    TArray2 b; b("x,B'") = gamma2("j,x") * (Fcn("c',j") * IBc("B',c'"));
    ...
    if (cabs_singles_h0_ == string("dyall_2")) {
      // - gamma(j,k) * h(k, beta) - gamma(jm,kl)*g(beta m, kl)
      b("x,B'") = - gamma2("j,k") * FmA("k,B'") - gamma4("j,m,k,l") * g("B',m,k,l");
    }
    ...
    TArray2 preconditioner = Delta_iA;
