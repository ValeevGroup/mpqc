...
namespace ci {
	...
	for (size_t l = 0, kl = 0; l < no; ++l) {
		...
		for (size_t j = 0, ij = 0; j < no; ++j) {
			...
			assert(fabs(V(index(j,i), index(k,l)) -
				    V(index(i,j), index(k,l)) < 1e-14));
    ...
    void integrals(const Matrix &C, const Matrix &h_ao,
		   Vector &h) {
	size_t no = C.rows();
	Matrix T1 = C*h_ao;
    ...
    void integrals(sc::Ref<sc::GaussianBasisSet> basis,
                   const Matrix &C,
		   sc::Ref<sc::OneBodyInt> int1,
                   Vector &h) {
	range shells(basis->nshell());
	size_t n = C.cols();
	Matrix h_ao(n,n);
