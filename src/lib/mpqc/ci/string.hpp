...
namespace ci {
    ...
    struct String {
        ...
        private:
            friend class String;
            explicit orbitals(int count) : count_(count) {}
            int data_[64];
            int count_;
        };
    ...
    /**
     * String::List represents a set of String objects. The objects are stored as a sequence that can be accessed
     * randomly. String::List also provides a String->index map.
    ...
    String::List<String::Index> strings(size_t no, size_t ne, size_t N = 0) {
	assert(no > ne);
	assert(ne > 0);
	std::vector<String> v;
	...
	do {
            ...
            //           << std::endl;
	    if (N && (String::difference(String(r), String(s))) > N) continue;
