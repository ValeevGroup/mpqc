...
#define MPQC_FILE_VERIFY(expr) BOOST_VERIFY((expr) >= 0)
    ...
    static hid_t h5_predtype() {
        ...
        BOOST_AUTO(pt, (make_map<int,double>
...
namespace mpqc {
    ...
    private:
        ...
        void initialize(const std::string &name, const Driver &driver) {
            ...
            if (!o) o = File::create(name, driver);
            Object::operator=(o);
        }
    ...
    struct File::Dataspace
        : range_operator_base<Dataspace<T_>*, Dataspace<T_>, Dataspace<const T_> > {

        typedef range_operator_base<
            Dataspace*, Dataspace<T_>, Dataspace<const T_> > range_base;
    ...
    private:
        ...
        Dataspace(const Object &parent, const std::vector<size_t> &base,
            ...
            : parent_(parent), range_base(this),
              base_(base), range_(r), ndims_(ndims) 
        {
            assert(ndims <= r.size());
            size_ = (r.size() ? 1 : 0);
    ...
    struct File::Dataset :
        File::Object,
        range_operator_base< Dataset<T>*, Dataspace<T>, Dataspace<const T> >
    {
        typedef range_operator_base<
            Dataset<T>*, Dataspace<T>, Dataspace<const T> > range_base;
        using range_base::operator();

        Dataset() : range_base(this) {}

        template<typename Extents>
        Dataset(const Object &parent,
		const std::string &name,
		const Extents &extents,
		const std::vector<size_t> &chunk = std::vector<size_t>())
...
#define MPQC_FILE_DATASET_OPERATOR(Z, N, TEXT)                                  \
    ...
    File::Group File::group(const std::string &name)  {
        return Group::create(*this, name);
    }
