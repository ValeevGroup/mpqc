...
std::vector<Cluster> individual_clustering(
    std::vector<Clusterable> const &c_ables) {
...
std::vector<std::pair<It, std::vector<It>>> init_ownership_map(
    std::vector<Clusterable> const &clusterables, const It first_heavy) {
...
template <typename It>
std::vector<Cluster> attach_to_owners(
    std::vector<std::pair<It, std::vector<It>>> const &h_owners) {
  std::vector<Cluster> cs;
...
std::vector<Cluster> attach_hydrogens::operator()(
