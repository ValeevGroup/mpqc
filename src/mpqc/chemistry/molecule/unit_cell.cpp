...
void UnitCell::print(std::ostream &os) const {
  ...
  os << "\tDirect lattice vector (in Bohr): [" << dcell_.transpose() << "]"
     << std::endl;
  os << "\tRange of expansion of Bloch Gaussians in AO Gaussians: ["
     << R_max_.transpose() << "]" << std::endl;
  os << "\tRange of Coulomb operation: [" << RJ_max_.transpose() << "]"
     << std::endl;
  os << "\tRange of density representation: [" << RD_max_.transpose() << "]"
     << std::endl;
  os << "\t# of k points in each direction: [" << nk_.transpose() << "]"
     << std::endl;
}
}  // mpqc namespace
