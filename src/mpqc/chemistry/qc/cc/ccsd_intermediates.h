...
// LCAO Integrals used physical noation <ij|ab>
...
class CCSDIntermediate {
 public:
  ...
  const DirectTwoElectronArray &direct_ao() const { return direct_ao_; }
  ...
  const TArray get_Xab() const {
    TArray result;
    TArray sqrt = lcao_factory_.atomic_integral().compute(L"(Κ|G| Λ)[inv_sqr]");
    TArray three_center = lcao_factory_.compute(L"(Κ|G|a b)");
  ...
  const TArray get_Xij() const {
    TArray result;
    TArray sqrt = lcao_factory_.atomic_integral().compute(L"(Κ|G| Λ)[inv_sqr]");
    TArray three_center = lcao_factory_.compute(L"(Κ|G|i j)");
  ...
  const TArray get_Xai() const {
    TArray result;
    TArray sqrt = lcao_factory_.atomic_integral().compute(L"(Κ|G| Λ)[inv_sqr]");
    TArray three_center = lcao_factory_.compute(L"(Κ|G|a i)");
  ...
  const TArray get_abij() {
    if (df_) {
      return lcao_factory_.compute(L"<a b|G|i j>[df]");
    } else {
  ...
  const TArray get_ijkl() {
    if (df_) {
      return lcao_factory_.compute(L"<i j|G|k l>[df]");
    } else {
  ...
  const TArray get_abcd() {
    if (df_) {
      return lcao_factory_.compute(L"<a b|G|c d>[df]");
    } else {
  ...
  const TArray get_iabc() {
    if (df_) {
      return lcao_factory_.compute(L"<i a|G|b c>[df]");
    } else {
  ...
  const TArray get_aibc() {
    if (df_) {
      return lcao_factory_.compute(L"<a i|G|b c>[df]");
    } else {
  ...
  const TArray get_ijak() {
    if (df_) {
      return lcao_factory_.compute(L"<i j|G|a k>[df]");
    } else {
  ...
  const TArray get_iajb() {
    if (df_) {
      return lcao_factory_.compute(L"<i a|G|j b>[df]");
    } else {
  ...
  TArray compute_u2_u11(const TArray &t2, const TArray &t1) {
    if (have_four_center_) {
      ...
      throw std::runtime_error(
          "CCSDIntermediate: integral-direct "
  ...
  TArray compute_u1a(const TArray &t1) {
    ...
    if (have_four_center_) {
      TArray u1a;
      u1a("p,r,i,j") =
          (Ca("q,a") * t1("a,i") * Ci("s,j")) * direct_ao_("p,q,r,s");
      return u1a;
    } else {
      throw std::runtime_error(
          "CCSDIntermediate: integral-direct "
  ...
  TArray compute_u1b(const TArray &t1) {
    ...
    if (have_four_center_) {
      TArray u1b;
      u1b("i,r,j,s") =
          (Ca("p,a") * t1("a,i") * Ci("q,j")) * direct_ao_("p,q,r,s");
      return u1b;
 ...
 private:
  /// LCAO Integral Object
  integrals::LCAOFactory<Tile, Policy> &lcao_factory_;
