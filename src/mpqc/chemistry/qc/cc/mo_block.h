...
class MOBlock {
 public:
  ...
  typedef std::pair<std::vector<std::size_t>, std::vector<std::size_t>>
      result_type;
  ...
  MOBlock(const mpqc::TRange1Engine &tre)
      : occ_range_(0ul, tre.get_occ_blocks()),
        vir_range_(tre.get_occ_blocks(), tre.get_all_blocks()),
        all_range_(0ul, tre.get_all_blocks()) {}

  /// Copy constructor

  /// deep copy
  MOBlock(const MOBlock &other)
