...
double fstg(double zeta, double x) { return -std::exp(-zeta * x) / zeta; }

double fngtg(const std::vector<double>& cc, const std::vector<double>& aa,
             double x) {
  double value = 0.0;
  const double x2 = x * x;
...
void LinearSolveDamped(int n, Eigen::MatrixXd& A, const double* b,
...
double ww(double x) { return wwtewklopper(x); }
}
...
std::vector<std::pair<double, double>> stg_ng_fit(std::size_t n, double zeta) {
  ...
  double lambda0 = 1000;  // damping factor is initially set to 1000, eventually
                          // should end up at 0
  const double nu =
      3.0;  // increase/decrease the damping factor scale it by this
  ...
  while (!converged && iter < maxniter) {
    ...
    for (unsigned int i = 0; i < npts; ++i) {
      const double x = xi[i];
      err[i] = (detail::fstg(zeta, x) - detail::fngtg(cc, aa, x)) *
  ...
  if (iter == maxniter && errnormI > 1e-10)
    throw std::runtime_error(
        "after the max # of iterations Levenberg-Marquardt failed to converge "
