...
/* /*! \brief struct which builds SchwarzScreen screeners *1/ */
...
inline double sqrt_l2Norm(Eigen::Map<const RowMatrixXd> const &map) {
  return std::sqrt(map.lpNorm<2>());
}

// Helper function to allow inf norm screening
inline double sqrt_inf_norm(Eigen::Map<const RowMatrixXd> const &map) {
  return std::sqrt(map.lpNorm<Eigen::Infinity>());
}
}  // namespace detail
...
class Qmatrix {
 private:
  ...
  using norm_func_type = decltype(detail::sqrt_inf_norm);
 ...
 public:
  ...
  Qmatrix(madness::World &world, E const &eng, Basis const &bs,
           norm_func_type norm_op = detail::sqrt_inf_norm) {
    // Get bs shells
  ...
  Qmatrix(TA::World &world, E const &eng, Basis const &bs0, Basis const &bs1,
    ...
    auto Q_func = [=](Shell const *sh0, Shell const *sh1, double *Q_val) {
      // Compute the shell set (a unit | a unit)
      eng->local().set_precision(0.);
      eng->local().compute(*sh0, *sh1, *sh0, *sh1);
      auto const &result = eng->local().results();
      const auto nsh0 = sh0->size();
      const auto nsh1 = sh0->size();
  ...
  /// Will return the largest value in Q_.row(a), this only depends on basis 0.
