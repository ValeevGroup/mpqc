    ...
    const TA::DistArray<Tile, Policy>& t1, bool df, std::string option)
{
  ...
  // one body
  {
    ...
    imds.FIJ = compute_cs_ccsd_F_OO(f_ij,f_ia,g_ijab_bar, g_ijka_bar, tau, t1);
    ...
    imds.FAB = compute_cs_ccsd_F_VV(f_ab,f_ia,g_ijab_bar,g_iabc_bar,tau,t1);
  }
  ...
  // two body
  {
    ...
    if(option=="ip"){
      ...
      imds.Wiajb = compute_cs_ccsd_W_ovov(imds.Wijka,g_iabc,g_ijab,g_iajb,t2,t1);

      imds.Wiabj = compute_cs_ccsd_W_ovvo(imds.Wijka,g_iabc,g_ijab_bar,g_ijab,g_iajb,t2,t1);

      imds.Wijkl = compute_cs_ccsd_W_oooo(g_ijkl, g_ijka, g_ijab, tau, t1);

      imds.Wiajk = compute_cs_ccsd_W_KaIj(lcao_factory,t1,t2,tau,imds.FIA,imds.Wijkl,df);
    }
    else{
      throw InputError("Wrong Option in compute_intermediates",__FILE__,__LINE__);
    }
