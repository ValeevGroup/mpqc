...
// "Root1": [1, 0, 2, 0, 1.0] (1st vector, electron excited from 1st occ. to 2nd
// vir.)
// "Root2": [1, 1, 1, 2, 1.0] (electrons excited from 1st occ. to 1st & 2nd
// vir.)
...
void EOM_CCSD::evaluate(ExcitationEnergy *ex_energy) {
  ...
  if (!this->computed()) {
    ...
    // do CCSD energy
    CCSD<TA::TensorD,TA::SparsePolicy>::evaluate(ccsd_energy.get());
    ...
    {
      auto cis = std::make_shared<CIS<TA::TensorD, TA::SparsePolicy>>(kv_);
...
MPQC_CLASS_EXPORT2("EOM-CCSD",mpqc::lcao::EOM_CCSD);