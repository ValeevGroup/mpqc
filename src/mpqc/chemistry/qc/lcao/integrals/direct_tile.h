...
class DirectTile {
 ...
 public:
  ...
  //  operator madness::Future<eval_type>() const {
...
class DirectDFArray {
 public:
  using Array = TA::DistArray<DirectDFTile<Tile, Builder>, Policy>;
 ...
 public:
  DirectDFArray() = default;
  DirectDFArray(std::shared_ptr<Builder> b, Array a)
      : builder_(std::move(b)), array_(std::move(a)) {}

  DirectDFArray(std::shared_ptr<Builder> b)
