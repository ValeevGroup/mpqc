...
inline Basis by_atom_basis(
    Molecule const &mol, Basis::Factory const &bs,
...
inline TA::TiledRange cadf_trange(Basis const &obs_by_atom,
  ...
  auto task = [&](unsigned long i, unsigned long j) {
    ...
    if (i == j) {
      ...
      if (C_df.is_local(eri3_idx_0) || C_df.is_local(eri3_idx_1)) {
        ...
        RowMatrixXd eri3_eig_0 = TA::eigen_map(
            eri3_tile_0, eri3_extent_0[0], eri3_extent_0[1] * eri3_extent_0[2]);

        TA::TensorD eri3_tile_1 = eri3.array().find(eri3_ord_1).get();
        RowMatrixXd eri3_eig_1 = TA::eigen_map(
            eri3_tile_1, eri3_extent_1[0], eri3_extent_1[1] * eri3_extent_1[2]);
...
void create_tiles(madness::World &world,
  ...
  auto two_ind_task = [&](integral_tile_type eri3_direct_tile0,
                          integral_tile_type eri3_direct_tile1,
                          TA::TensorD M_tile00, TA::TensorD M_tile01,
                          TA::TensorD M_tile10, TA::TensorD M_tile11,
...
compute_atomic_fitting_coeffs(
    ...
    Molecule const &dfbs_molecule, BasisFactory const &obs_set,
    BasisFactory const &dfbs_set, MetricEngine const &eng,
    std::unordered_map<std::size_t, std::size_t> &obs_atom_to_cluster_map,
  ...
  const auto dfbs_array =
      std::vector<Basis>{{by_atom_dfbs, by_atom_dfbs}};
  ...
  auto three_c_array =
      std::vector<Basis>{{by_atom_dfbs, by_atom_obs, by_atom_obs}};
