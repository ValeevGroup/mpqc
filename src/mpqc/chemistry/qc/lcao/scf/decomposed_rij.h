...
template <typename Tile, typename Policy,
          typename array_type = TA::DistArray<Tile, Policy>>
array_type take_row_from_2D_array(TA::DistArray<Tile, Policy>& matrix,
                                  int64_t row_idx) {
  ...
  TA::TensorF norms(trange.tiles_range(), trange.elements_range().volume() /
                                              trange.tiles_range().volume());
  TA::SparseShape<float> shape(world, norms, trange);

  array_type result(world, trange, shape);
...
template <typename Tile, typename Policy,
          typename TArray = TA::DistArray<Tile, Policy>>
TArray take_col_from_2D_array(TA::DistArray<Tile, Policy>& matrix,
                              int64_t col_idx) {
