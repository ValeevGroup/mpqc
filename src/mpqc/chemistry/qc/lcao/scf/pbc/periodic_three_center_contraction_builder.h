				...
				for (auto tileR = 0ul; tileR != ntilesR; ++tileR) {
					...
					for (auto RJ = 0; RJ != RJ_size_; ++RJ, ++tile012) {
						if (pmap->is_local(tile012))
							WorldObject_::task(
										me, &PeriodicThreeCenterContractionBuilder_::compute_task_Xmn_mn,
										D_0R, norm_D_0R, RJ,
  ...
  void init() {
	...
	void accumulate_local_task(Tile arg_tile, long tile_ord) {
		...
		if (!local_contr_tiles_.insert(
						acc,
						std::make_pair(tile_ord, arg_tile))) {  // CRITICAL SECTION
	...
	void compute_task_Xmn_mn(Tile D_0R, Tile norm_D_0R, size_t RJ, std::array<size_t, 3> tile_idx) {
							...
							for (auto f_aux = 0, f_aux_0R = 0; f_aux != nf_aux; ++f_aux) {
								...
								for (auto f0 = 0; f0 != nf0; ++f0) {
									...
									for (auto fR= 0; fR != nfR; ++fR, ++f_aux_0R) {
										...
										result_ptr[cf_aux] += (D_0R_ptr != nullptr)
												? D_0R_ptr[cf0R] * value
												: 0.0;
									}
								}
