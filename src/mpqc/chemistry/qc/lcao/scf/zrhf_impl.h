...
zRHF<Tile, Policy>::zRHF(const KeyVal& kv) : PeriodicAOWavefunction<Tile, Policy>(kv), kv_(kv) {}
...
template<typename Tile, typename Policy>
zRHF<Tile, Policy>::array_type zRHF<Tile, Policy>::compute_density() {
	auto& ao_factory = this->ao_factory();
	auto& world = ao_factory.world();
...
template<typename Tile, typename Policy>
bool zRHF<Tile, Policy>::can_evaluate() {
	// can only evaluate the energy
	return energy->order() == 0;
}

template<typename Tile, typename Policy>
void zRHF<Tile, Policy>::evaluate(Energy* energy) {
	if (!this->computed()) {
		init(kv_);
...
template<typename Tile, typename Policy>
zRHF<Tile, Policy>::array_type zRHF<Tile, Policy>::J_builder() {
	return this->ao_factory().compute_direct(L"(μ ν| J|κ λ)");
}

template<typename Tile, typename Policy>
zRHF<Tile, Policy>::array_type zRHF<Tile, Policy>::K_builder() {
