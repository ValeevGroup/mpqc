...
double
Unit::to(const Unit& unit) const
{
  return to_atomic_units_/unit.to_atomic_units_;
}

double
Unit::from(const Unit& unit) const
{
  return 1.0/to(unit);
}

double
Unit::to_atomic_units() const
{
  return to_atomic_units_;
}

double
Unit::from_atomic_units() const
{
...
void Unit::parse() {
  ...
  while (rest) {
    ...
    if (eq(unitstring, "bohr") || eq(unitstring, "bohrs")) {
      ...
      factor = 1.0e-5 * a0 / Ea;
    } else if (eq(unitstring, "m") || eq(unitstring, "meter") ||  eq(unitstring, "meters")) {
      factor = 1.0 / a0;
      ...
      factor = M_PI / 180.0;
    } else if (eq(unitstring, "second") || eq(unitstring, "seconds") || eq(unitstring, "s")) {
      factor = Ea / hbar;
    } else {
...
UnitFactory::UnitFactory(std::string version) {
  if (version == "CODATA2014") {
    constants_ = std::make_shared<
        FundamentalConstants<constants::codata_2014<double>>>();
  } else if (version == "CODATA2010") {
    constants_ = std::make_shared<
        FundamentalConstants<constants::codata_2010<double>>>();
  } else if (version == "MPQC2") {
    constants_ =
        std::make_shared<FundamentalConstants<constants::mpqc2<double>>>();
  } else {
    throw mpqc::InputError("Unknown version of UnitFactory requested", __FILE__,
                           __LINE__, "version", version.c_str());
  }
}
