...
std::array<TiledArray::Tensor<T, AT>, 2> eigen_estimator(
    std::array<TiledArray::Tensor<T, AT>, 2> &result,
    TiledArray::Tensor<T, AT> const &tile) {
  typedef typename TiledArray::Tensor<T, AT>::size_type size_type;

  if (result[0].empty()) {
    // Construct result tensors
    const std::array<size_type, 1> range_start = {{tile.range().lobound()[0]}};
    const std::array<size_type, 1> range_finish = {{tile.range().upbound()[0]}};
  ...
  size_type const finish_min = *std::min_element(finish.begin(), finish.end());
...
void add_to_diag(
  ...
  for (auto it = A.begin(); it != end; ++it) {
    ...
    auto diagonal_tile = std::all_of(
        idx.begin(), idx.end(),
        [&](typename std::remove_reference<decltype(A)>::type::size_type const
                &x) { return x == idx.front(); });
...
template <typename Array>
std::array<typename Array::value_type::numeric_type, 2> eval_guess(
    Array const &A) {
  A.world().gop.fence();
  ...
  for (auto i = 0ul; i < A.trange().tiles_range().extent()[0]; ++i) {
    ...
    local_reduce.add(
        A.world().gop.all_reduce(key, pair, global_accumlator{range}));
  }
