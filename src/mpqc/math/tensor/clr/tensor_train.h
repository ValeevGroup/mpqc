...
std::array<RowMatrixXd, 4> TT_TA_ARRAY(
  ...
  Eigen::JacobiSVD<RowMatrixXd> svd(A_eig,
  ...
  // R = (r0, ajb)
  RowMatrixXd sig =
      RowMatrixXd(svd.singularValues().asDiagonal()).block(0, 0, rank0, rank0);
  ...
  // R1 = (r1, jb)
  sig =
      RowMatrixXd(svd.singularValues().asDiagonal()).block(0, 0, rank1, rank1);
  ...
  // G3 = (r2, b)
  sig =
      RowMatrixXd(svd.singularValues().asDiagonal()).block(0, 0, rank2, rank2);
  G3 = sig * svd.matrixV().leftCols(rank2).transpose();

  return {{G0, G1, G2, G3}};
}

std::array<RowMatrixXd, 4> TT_one_over_delta(Eigen::VectorXd const &e_vals,
                                             int64_t occ) {
  const auto size = e_vals.size();
  const auto vir = size - occ;
  ...
  Eigen::JacobiSVD<RowMatrixXd> svd(D,
                                    Eigen::ComputeThinU | Eigen::ComputeThinV);
  svd.setThreshold(1e-3);
...
TA::DistArray<TA::TensorD, TA::SparsePolicy> make_Wir0k(
    madness::World &world, RowMatrixXd const &G0, RowMatrixXd const &W,
...
TA::DistArray<TA::TensorD, TA::SparsePolicy> make_Ycr2b(
    madness::World &world, RowMatrixXd const &G3, RowMatrixXd const &Cv,
...
TA::DistArray<TA::TensorD, TA::SparsePolicy> make_Wr1lr2j(
    madness::World &world, RowMatrixXd const &G2, RowMatrixXd const &W,
      ...
      for (auto l = lo_bound[1]; l < hi_bound[1]; ++l) {
        for (auto r2 = lo_bound[2]; r2 < hi_bound[2]; ++r2) {
          for (auto j = lo_bound[3]; j < hi_bound[3]; ++j) {
            for (auto z = 0; z < occ; ++z) {
              auto r1z = r1 * occ + z;
              tile(r1, l, r2, j) += W(z, l) * G2(r1z, r2) * W(z, j);
            }
          }
        }
      }
...
TA::DistArray<TA::TensorD, TA::SparsePolicy> make_Yr0ar1d(
    madness::World &world, RowMatrixXd const &G1, RowMatrixXd const &Cv,
    RowMatrixXd const &Q, TA::TiledRange1 const &pao_trange, int64_t vir,
      ...
      for (auto d = lo_bound[1]; d < hi_bound[1]; ++d) {
        for (auto r1 = lo_bound[2]; r1 < hi_bound[2]; ++r1) {
          for (auto a = lo_bound[3]; a < hi_bound[3]; ++a) {
            for (auto z = 0; z < vir; ++z) {
              auto r0z = r0 * vir + z;
              tile(r0, d, r1, a) += Cv(d, z) * G1(r0z, r1) * Q(z, a);
            }
          }
        }
      }
...
std::array<TA::DistArray<TA::TensorD, TA::SparsePolicy>, 4> transform_tensors(
    Eigen::VectorXd const &e_vals, int64_t occ, madness::World &world,
    TA::TiledRange1 const &occ_trange, TA::TiledRange1 const &vir_trange,
    RowMatrixXd const &Uocc, RowMatrixXd const &Cv, RowMatrixXd const &Q) {
